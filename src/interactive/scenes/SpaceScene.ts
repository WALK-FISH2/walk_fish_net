...
const starSeeds = Array.from({ length: 240 }, (_, index) => ({ x: ((index * 67) % 239) / 238, y: ((index * 97) % 233) / 232, size: index % 31 === 0 ? 3 : index % 9 === 0 ? 2 : 1, phase: index * 0.37, layer: index % 3 }));
...
export class SpaceScene implements PixelScene {
  ...
  update({ width, height, progress, time, particleScale }: SceneFrame) {
    ...
    for (let index = 0; index < activeStars; index += 1) {
      ...
      const speed = [3, 7, 13][star.layer];
      const x = ((star.x * width + progress * speed * 9 + time * speed * 0.12) % width + width) % width;
      const y = ((star.y * height + progress * speed * 3) % height + height) % height;
      const alpha = 0.28 + star.layer * 0.17 + (Math.sin(time * (0.5 + star.layer * 0.2) + star.phase) + 1) * 0.11;
      drawPixelStar(this.midground, Math.round(x), Math.round(y), star.size, star.layer === 2 ? 0xfff3c4 : 0xdad9ff, alpha);
    }
    ...
    drawFloatingIsland(this.nearDecor, width * 0.73 + Math.sin(time * 0.22) * 10, height * 0.63 + Math.cos(time * 0.3) * 7, Math.max(0.75, Math.min(1.15, width / 1100)));
    const meteorX = ((time * 43 + progress * width * 0.35) % (width + 180)) - 90;
    this.nearDecor.moveTo(meteorX, height * 0.18).lineTo(meteorX - 58, height * 0.12).stroke({ width: 3, color: 0x89f0d7, alpha: 0.55 });
    drawPixelStar(this.nearDecor, meteorX, height * 0.18, 3, 0xfff3c4, 0.9);

    this.particles.clear();
    for (let index = 0; index < Math.round(50 * particleScale); index += 1) {
      const x = ((index * 113 + time * (3 + index % 4)) % (width + 40)) - 20;
      const y = ((index * 59 + progress * 120 + Math.sin(time + index) * 8) % height + height) % height;
