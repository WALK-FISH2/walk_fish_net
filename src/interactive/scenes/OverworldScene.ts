...
export class OverworldScene implements PixelScene {
  ...
  update({ width, height, progress, time, particleScale }: SceneFrame) {
    ...
    const farShift = progress * width * 0.07;
    const midShift = progress * width * 0.19;
    const nearShift = progress * width * 0.4;
    ...
    for (let index = -1; index < 4; index += 1) {
      const x = index * 390 - ((farShift * 0.7) % 390);
      drawMountain(this.farDecor, x, groundY + 5, 420, 230 + (index % 2) * 48, index % 2 ? 0x4f8b88 : 0x6ba4a0);
    }
    for (let index = -1; index < 4; index += 1) {
      const x = index * 360 + 130 - ((farShift * 1.5 + time * 9) % 360);
      drawPixelCloud(this.farDecor, x, 150 + (index % 3) * 34, 1, 0xffffff, 0xd7ece5);
    }

    this.midground.clear();
    this.midground.circle(width * 0.1 - midShift * 0.3, groundY + 80, 210).fill(0x78c868)
      .circle(width * 0.55 - midShift * 0.35, groundY + 100, 260).fill(0x61b75c)
      .circle(width * 0.95 - midShift * 0.28, groundY + 60, 190).fill(0x4aa657);
    drawTower(this.midground, width * 0.69 - midShift * 0.55, groundY, Math.max(0.75, Math.min(1.25, width / 1100)));
    ...
    const pathStart = width * 0.02 - nearShift * 0.16;
    this.nearDecor.moveTo(pathStart, height).lineTo(width * 0.28 - nearShift * 0.06, groundY + 25).lineTo(width * 0.48 - nearShift * 0.03, groundY + 25).lineTo(width * 0.78, height).fill(0xd29a5f);
    ...
    for (let index = 0; index < 18; index += 1) {
      const x = ((index * 83 - nearShift * 0.7) % (width + 80) + width + 80) % (width + 80) - 40;
      drawFlower(this.nearDecor, x, groundY + 12, index % 2 ? 0xf45b5b : 0xffd25a, index % 4 === 0 ? 1.5 : 1);
    }

    const travelerX = Math.round(width * 0.13 + progress * width * 0.46);
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      const x = ((dust.x * width - nearShift * (0.3 + index % 3 * 0.08)) % width + width) % width;
    ...
    for (let index = -1; index < Math.ceil(width / 54) + 2; index += 1) {
      const x = index * 54 - (nearShift * 1.35 % 54);
