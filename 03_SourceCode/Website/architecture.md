...
- `src/types/content.ts` 定义 `ProgramSummary`、`ProgramDetail`、`ProgramPlatform`、`ProgramMedia`、状态与隐私结构；
- `src/lib/content.ts` 使用 `CollectionEntry<"programs">`；
- 内容存放在 `src/content/programs/`；
- 主路由是 `/programs` 与 `/programs/[slug]`；
- 每项 Program 必须包含本人贡献、限制、隐私、演示类型和八项必备详情内容；
- `ProgramSummary` 只携带列表/首页需要的平台种类与标签，`ProgramDetail` 才携带完整外链、二维码与媒体，避免详情媒体进入首页 hydration 数据；
- `platforms[]` 支持 HTTPS 网页版和带 alt/说明的微信小程序码，`media[]` 支持视频、GIF、截图及横竖屏方向；
- 详情页使用普通文档流：桌面为 `1.15fr / minmax(280px, 0.85fr)` 双栏，375px 为“标题/入口 → 媒体 → 详情”单列；
...
### M6.1 已实施：深海 Programs 左右分栏与标题渐隐

当前运行代码在保留 M8-10 防重叠基线和 DOM 源码顺序的前提下，为桌面完整动画增加两个隔离的内容轨道：

- 左侧标题轨道承载“沉在海里的程序档案”，使用 Programs section 边界内的受限 sticky，并停驻在避开浮动导航的左上安全区；
- 右侧卡片轨道继续承载现有 `.portholes` 和三个 `.porthole`；卡片容器整体右移，但内部仍是当前完整纵向 Grid、显式 gap 和普通文档流；
- 单张卡片不新增 sticky、fixed、独立步骤容器或接棒式显隐；terminal、probe、capsule 的现有排列、内容和自然出场保持不变；
- 最后一张卡片进入 Programs 尾段时，标题通过独立局部进度逐渐变暗并降低透明度，在卡片离开主要阅读区时完全消失；反向滚动恢复颜色和透明度；
- 两条轨道使用集中配置的宽度、gap、顶部安全偏移和标题退场参数；标题轨道宽度上限为 `340px`，轨道 gap 为 `48–72px`，标题安全顶部为 `112–176px`，1280px 桌面额外预留 `84px` 右侧 fixed UI 安全区；旧的 `--porthole-index` sticky top 递增堆栈没有恢复；
- 375px、内容宽度不足和简化动画继续使用当前标题在上、三张卡片在下的普通文档流；
- DOM、Tab 和屏幕阅读器顺序保持标题 → 卡片 01 → 卡片 02 → 卡片 03 → 总入口，CSS 分栏不重排源码；
- 不新增第二个 ScrollTrigger、页面级 pin、scroll lock 或新的进度源；现有 `globalProgress`、`0.430–0.640` Programs 窗口、M5 边界、Canvas fallback 和世界 `0°` 不变量保持不变。
