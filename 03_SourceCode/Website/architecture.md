...
- 模式切换先记录 `globalProgress`，等待布局与 `ScrollTrigger.refresh()` 完成，再在下一帧滚动到新高度下的等价坐标；80.0073% 的双向切换实测进度差为 0；
...
- M3 陆地视差、M4 下潜/深海、M4.5 陆海翻涌、M5 气泡到繁星/流星、M5.5 视觉抛光与 M6 星空/动效模式入口均已完成正式验收。
