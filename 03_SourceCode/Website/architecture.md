...
最新 M3 验收结果为：构建成功、15 个 HTML、类型检查无错误/警告/提示、lint 通过、6 项静态输出与源码边界测试通过。浏览器在 1280×720 下记录陆地 0%、25% 和 25%→0% 回滚；在 375×812 下横向溢出为 0；Reduced Motion 下媒体查询命中、故事高度为 `auto`；文章入口可指针导航并获得键盘焦点；Canvas 内容边界测试通过；修复初始化竞态后的浏览器 warn/error 列表为空。既有静态路由、子路径、旧路由目标、canonical 与 Sitemap 结论保持不变。
...
- 场景内部仍有部分硬编码过渡时序和未完成的对象池目标；陆地四层视差已经完成集中配置与正式验收。
