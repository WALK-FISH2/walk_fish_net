...
空的演示或源码地址不会渲染按钮；生产构建会排除 `draft: true`。M7 已在保持 `demoType`/`demoUrl` 兼容的基础上增加可选 `platforms`/`media`，详情页可以组合展示真实网页版、竖屏视频与微信小程序码。“打开网页版”紧跟标题和简介，位于长篇正文之前。已经独立上线的程序不要求复制到 `Project_Demos`；完整规则见 `docs/product/content-model.md` 与 `docs/product/m7-real-program-showcase-spec.md`。
...
当前已完成第二轮核心视觉升级、M4.5 陆海三层浪、M5/M5.5 海空粒子与星空抛光、M6 动效模式、M6.1 深海 Programs 布局、M6.2 跨世界小章鱼主题角色、M6.3 普通内容页原生纵向滚动修复，以及 M7 真实 Programs 展示。“拉了么”已作为首页第一项接入真实网页版、H.264 竖屏演示、poster、微信小程序码和完整的贡献/限制/隐私说明；普通、潜水和宇航员三种生产形态由同一进度模型串联，文章、Programs、关于页和 404 均可自然滚动。MDX 自定义交互组件、代码 token 高亮、RSS 和音频仍尚未加入。
