...
- [x] `M7-14` 运行 Astro Check、ESLint、测试、生产构建和静态路由验证（2026-08-09 实际证据：Astro Check 45 文件 0 errors/warnings/hints；ESLint 通过；19/19 测试；`npm run build` 与 `npm run build:sites` 无警告通过；17/17 静态路由 HTTP 200，未知路由 404，`dist/server` 不存在）
- [x] `M7-15` 保持海洋区域三张 Program 卡片并验证四项公开内容、Projects 兼容路由、Sitemap 与 17 个静态 HTML（代码/构建证据：首页恰有 3 个 `.porthole`，按“拉了么 → 像素漫游个人站 → Tidy Desk”排序；`Signal Garden` 继续在列表公开；4 个 canonical Program 与 5 个 Projects 兼容页均生成；Sitemap 收录四个 canonical Program且排除 `/projects`）
- [x] `M7-16` 接入并验证“拉了么”生产媒体（媒体/命令证据：`demo.mp4` 为 H.264 High Profile、576×1280、22.133 秒；poster 为 1080×2210 WebP；小程序码为 258×258 PNG；三项静态请求均 200 且 MIME 正确；HEVC 备份未进入 `dist/` 或 `sites-dist/`）
...
- [x] `M8-11` 首页陆地卡片提前离场、海洋卡片延后进入（文章卡片仍在 `0.285–0.368` 最多上移 `56vh`；Programs 将进度增长/保持/释放改为恒定桌面进入偏移，三卡只随页面单向上移，不再出现先下推、后回收的反向运动；原 `54vh` 已证明该机制单调无反转，项目所有者随后手动微调为当前 `34vh`，自动测试同步锁定实际参数；Astro Check、ESLint、测试和 15 页生产构建通过）
- [x] `M8-12` Programs 列表雷达扫描目标抛光（代码/测试/浏览器证据：雷达当前固定维护鱼、`</>`、`{ }`、猫脸、`>_` 五个几何/字符节点，小鱼按参考图使用扁菱形轮廓鱼身、小比例三角轮廓尾和较大单点眼；扫描命中后点亮并在随机 `1.1–2.1s` 内渐隐，熄灭后在安全半径重新随机定位；最初四目标版本已在桌面、375px、Reduced Motion 和离开/返回场景验证循环与布局，项目所有者后续增加猫脸节点，当前静态输出与自动测试同步确认五节点且仍只有一个活动扫描；控制台 0 warning/error；Astro Check 0/0/0、ESLint、完整测试和 15 页生产构建通过）
- [x] `M8-13` 普通内容页顶部导航固定（代码/浏览器/命令证据：`.page-shell > .site-nav` 只增加 `position: sticky; top: 0`，继续占据文档流；文章列表/详情、Programs 列表/详情、关于页在桌面顶部/中段/底部/向上返回均为 `top=0`，高度恒为 `78px`；375px 高度恒为 `69px`、横溢为 0，菜单完整覆盖视口且关闭后恢复滚动；首页仍为独立 `fixed top=14px`；控制台为空，Astro Check 0/0/0、ESLint、完整测试和 15 页生产构建通过）
- [x] `M8-14` 将“花光马斯克的钱”接入 Programs（内容/构建/测试证据：新增 `spend-musk-money` 真实外部 Program，公开 M0 原型、自由/限时挑战、商品筛选、统计、成就、本地存档、限制与隐私边界；首要外链使用用户提供的 HTTPS 地址并具备新标签页安全属性；`featured: true` 后与“拉了么”“CRT转换器”组成首页当前三卡，用户已设为草稿的 `pixel-journey` 未被改回；生成 canonical 与 Projects 兼容详情，Sitemap 只收录 canonical；23/23 自动化测试和 15 页静态构建通过）
