...
- [x] `M8-11` 首页陆地卡片提前离场、海洋卡片延后进入（文章卡片仍在 `0.285–0.368` 最多上移 `56vh`；Programs 根据项目所有者录像将进度增长/保持/释放改为固定 `54vh` 桌面进入偏移，三卡只随页面单向上移，不再出现先下推、后回收的反向运动。本地 1700×926 在 40%→66% 的卡片 top 为 `926/715/433/222/-411/-587/-763/-904px`，严格单调上移，控制台 0 warning/error；Astro Check 0/0/0、ESLint、21/21 测试和 15 页生产构建通过）
- [x] `M8-12` Programs 列表雷达扫描目标抛光（代码/测试/浏览器证据：雷达固定维护鱼、`</>`、`{ }`、`>_` 四个几何/字符节点，小鱼按参考图使用扁菱形轮廓鱼身、小比例三角轮廓尾和较大单点眼；扫描命中后点亮并在随机 `1.1–2.1s` 内渐隐，熄灭后在安全半径重新随机定位；1280×720 停留 7 秒后四项坐标均变化，375×812 四项均在 `180px` 圆内且横溢为 0，Reduced Motion 同样持续扫描和换位；历史离开/返回仍只有四节点和一个活动扫描；新建页面控制台 0 warning/error；Astro Check 0/0/0、ESLint、雷达自动测试和 15 页生产构建通过）
