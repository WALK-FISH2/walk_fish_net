...
Program 可以链接到独立部署的真实应用：纯前端应用可以使用任意静态托管，需要后端的应用可以使用自己的服务器。只要主站详情页只输出链接、二维码、媒体和说明，不代理外部 API、不增加 Astro SSR adapter，该外部应用就不是主站的 Node 生产运行时依赖。

- 已上线程序不要求复制到 `Project_Demos` 或打入 `dist/`；
- 外部 Program 使用真实 HTTPS 地址和新标签页安全属性；
- 大视频优先引用独立媒体托管地址，并只在 Program 详情页加载；
- 二维码、poster 和优化后的截图可以随主站作为静态资源构建；
- 首页构建产物不得包含全部 Program 媒体或演示代码；
