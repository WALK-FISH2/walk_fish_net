...
- 静态输出测试确认 `/programs/` 恰好生成鱼、`</>`、`{ }`、`>_` 四个目标，雷达仍为 `aria-hidden`，且源码没有 Reduced Motion 禁用分支；
