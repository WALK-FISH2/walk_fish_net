...
项目所有者后续把 `programCardsEntryOffset.desktopVh` 从 `54` 手动调整为 `34`。这只缩短桌面卡片的恒定延后距离，不改变“全区间保持常量、不得增长或释放”的单向机制。当前交互、视觉、验收、测试文档和自动化期望均以代码实际值 `34vh` 为准；16.3 保留为 `54vh` 时验证单向机制的历史证据。

## 17. 2026-08-13 Programs 列表雷达扫描目标

### 17.1 修改范围

本次只抛光 `/programs/` Hero 中既有雷达，不修改首页场景、Program 内容模型、路由、搜索筛选逻辑或卡片版式。雷达保留原 `250px` 桌面尺寸、`180px` 移动尺寸、同心圆、中心点和扫描棒。初版新增四个几何/字符目标：小鱼、`</>`、`{ }`、`>_`；项目所有者后续手动增加猫脸目标，当前共五个。小鱼最终按项目所有者参考图调整为 `20×12px` 扁菱形轮廓鱼身、`7×10px` 三角轮廓尾和 `3×3px` 同色眼点；尾部小于鱼身，鱼身与鱼尾内部使用雷达底色镂空，不保留实心块、圆角或弧线。

实现由 `SonarRadar.astro` 维护五个固定 DOM 节点，由 `sonarRadar.ts` 集中维护参数和纯函数。原 CSS `4s` keyframes 扫描改为同周期的单一 RAF 扫描，以便用同一角度完成准确命中；目标熄灭后只更新节点位置，不增删节点。

| 参数 | 值 |
| --- | --- |
| 目标数量 | 固定 `5` |
| 目标类型 | `fish`、`tag`、`braces`、`cat`、`terminal` |
| 扫描周期 | `4000ms` |
| 单目标渐隐 | 随机 `1100–2100ms` |
| 随机半径 | 雷达半径的 `22%–68%` |
| 目标最小距离 | 雷达直径归一化 `0.20`（桌面约 `50px`） |
| 颜色 | `var(--sea-cyan)`，与最外圈一致 |
| Reduced Motion | 保留完整扫描、命中、渐隐和重新定位 |

生命周期：`visibilitychange` 隐藏时暂停 RAF，显示时恢复；`pagehide` 非 BFCache 离场或组件清理时取消 RAF、移除监听器并清除实例清理句柄；初始化前先调用旧清理句柄，避免 HMR 或重复进入叠加循环。雷达保持 `aria-hidden="true"` 与 `pointer-events: none`。

### 17.2 附带控制台修复

浏览器初验发现 Program “拉了么”的 `stack + tags` 中“微信小程序”重复，`ContentFilter` 以标签文字作为 React key 时产生重复 key 错误。仅将 key 改为“标签文字 + 当前索引”，没有去重、改字、重排或改变任何视觉样式；新建页面复验控制台为空。

### 17.3 验证证据

| 验证 | 结果 |
| --- | --- |
| Astro Check | 47 个文件，0 errors / 0 warnings / 0 hints |
| ESLint | 通过 |
| 雷达自动测试 | 通过；覆盖四目标静态输出、扫描回绕、随机时长边界、重新定位与 cleanup 源码约束 |
| 生产构建 | Astro static build 成功，`dist/` 仍生成 15 个 HTML |
| 1280×720 停留测试 | 4 个目标；停留 7 秒后四项目标坐标均发生变化；横向溢出为 0 |
| 375×812 | 雷达为 `180×180px`；4 个目标都在圆内；横向溢出为 0 |
| Reduced Motion | `data-motion-mode=reduce` 下扫描角继续变化，停留 7 秒后四项目标继续重新定位 |
| 离开与返回 | `/programs/ → /about/ → 返回` 后仍为 4 个节点，500ms 内扫描角由 `0°` 继续前进，没有重复实例 |
| 浏览器控制台 | 新建页面、Reduced Motion 与历史返回均为 0 warnings / 0 errors |

本次验证只使用本地 `http://localhost:4321/`，未发布到 Cloudflare。上表的浏览器行记录的是项目所有者增加猫脸前的四目标版本；后续静态输出和完整自动测试已按当前五节点同步，雷达动画机制未改变。

## 18. 2026-08-13 普通内容页顶部导航固定

### 18.1 修改范围

文章列表/详情、Programs 列表/详情、关于页和同一普通页面外壳内的 404 继续共用原 `SiteNav.astro`。本次只在 `.page-shell` 作用域增加：
...
导航仍占据正常文档流，因此首屏正文无需增加人工补位。没有修改导航的宽度、高度、padding、颜色、边框、阴影、品牌、桌面链接或移动菜单样式；没有新增滚动监听、组件状态、嵌套滚动容器或第二套导航。沉浸式首页不含 `.page-shell`，继续使用原 `.site-nav--floating { position: fixed; top: 14px; }`。
...
| Astro Check | 47 个文件，0 errors / 0 warnings / 0 hints |
| ESLint | 通过 |
| 自动化专项 | 普通页面纵向滚动与菜单恢复测试通过；新增 sticky 作用域和“不覆盖视觉属性”断言 |
| 生产构建 | Astro static build 成功，`dist/` 仍生成 15 个 HTML |
| 桌面五类页面 | `/articles/`、文章详情、`/programs/`、Program 详情、`/about/` 在顶部/中段/底部/向上返回时导航 `top=0`、高度恒为 `78px` |
| 桌面原样式 | land/sea/space 导航背景和 3px 底边颜色保持各主题现有计算值；五页横向溢出均为 0 |
| 375×812 | Programs 导航顶部/中段/底部均 `top=0`，高度恒为 `69px`，横向溢出为 0 |
| 移动菜单 | 打开后面板覆盖 `0–812px` 视口并锁定 body；关闭后 inline overflow 恢复为空、导航仍 `top=0`、滚动位置保持 |
| 首页回归 | body 不含 `.page-shell`；导航仍为 `.site-nav--floating`、`position: fixed`、`top=14px` |
