...
本次只抛光 `/programs/` Hero 中既有雷达，不修改首页场景、Program 内容模型、路由、搜索筛选逻辑或卡片版式。雷达保留原 `250px` 桌面尺寸、`180px` 移动尺寸、同心圆、中心点和扫描棒，新增四个几何/字符目标：小鱼、`</>`、`{ }`、`>_`。小鱼最终按项目所有者参考图调整为 `20×12px` 扁菱形轮廓鱼身、`7×10px` 三角轮廓尾和 `3×3px` 同色眼点；尾部小于鱼身，鱼身与鱼尾内部使用雷达底色镂空，不保留实心块、圆角或弧线。

实现由 `SonarRadar.astro` 维护四个固定 DOM 节点，由 `sonarRadar.ts` 集中维护参数和纯函数。原 CSS `4s` keyframes 扫描改为同周期的单一 RAF 扫描，以便用同一角度完成准确命中；目标熄灭后只更新节点位置，不增删节点。

| 参数 | 值 |
| --- | --- |
| 目标数量 | 固定 `4` |
| 目标类型 | `fish`、`tag`、`braces`、`terminal` |
| 扫描周期 | `4000ms` |
...
本次验证只使用本地 `http://localhost:4321/`，未发布到 Cloudflare。
